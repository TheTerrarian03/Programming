...
def update_leaderboard(file_name, leader_names, leader_scores,  player_name, player_score):
  ...
  leaderboard_all_file = open(file_name[:-4]+"_ALL" + file_name[-4:], "w")
  for index in range(len(leader_names)):
    leaderboard_all_file.write(leader_names[index] + "," + str(leader_scores[index]) + "\n")
  leaderboard_all_file.close()

  # TODO 11: keep both lists at 5 elements only (top 5 players)
  while True:  # run until i tell it to stop
    # if the lists have more than 5 items, pop the last item[s]
    if len(leader_names) > 5:
      leader_names.pop()
      leader_scores.pop()
    # otherwise, break out of the infinite while loop
    else:
      break
  ...
  for index in range(len(leader_names)):  # either names of score would work
...
if __name__ == "__main__":
  ...
  """print(get_names(file_name=file_name))
  print(get_scores(file_name=file_name))"""
  update_leaderboard(file_name, get_names(file_name=file_name), get_scores(file_name=file_name), "pekay", 4)
  update_leaderboard(file_name, get_names(file_name=file_name), get_scores(file_name=file_name), "dylan", 10)
  update_leaderboard(file_name, get_names(file_name=file_name), get_scores(file_name=file_name), "noah", 7)
  update_leaderboard(file_name, get_names(file_name=file_name), get_scores(file_name=file_name), "logan", 100)
  # print(file_name[:-4]+"_ALL" + file_name[-4:])